...
def iris_position(iric_center, right_point, left_point, top_point, down_point):
    ...
    if ratio_vertical <= 0.55:
        iris_position += " TOP"
    elif ratio_vertical > 0.55 and ratio_vertical <= 0.60:
...
def winking(
    ...
    # print("ration right", ratio_right, " ratio left", ratio_left)
...
with mp_face_mesh.FaceMesh(
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            # right eye
            # yellow
            cv.circle(frame, mesh_points[R_H_TOP][0], 3, (0, 255, 255), -1, cv.LINE_AA)
            ...
            cv.circle(
                frame, mesh_points[RIGHT_EYE_DOWN][0], 3, (0, 43, 255), -1, cv.LINE_AA
            )

            #  left eye
            # brown
            """
            cv.circle(frame, mesh_points[L_H_TOP][0], 3, (93, 86, 155), -1, cv.LINE_AA)
            cv.circle(frame, mesh_points[L_H_DOWN][0], 3, (93, 86, 155), -1, cv.LINE_AA)
            cv.circle(frame, mesh_points[L_H_LEFT][0], 3, (93, 86, 155), -1, cv.LINE_AA)
            cv.circle(
            ...
            cv.putText(
                frame,
                f"Horizontal direction {iris_pos.split()[0]}",
                (30, 30),
                cv.FONT_HERSHEY_PLAIN,
                1.2,
                (0, 255, 0),
            ...
            cv.putText(
                frame,
                f"Vertical direction {iris_pos.split()[1]}",
                (30, 50),
                cv.FONT_HERSHEY_PLAIN,
                1.2,
                (0, 255, 0),
